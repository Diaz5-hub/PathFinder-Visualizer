...
def algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():  ##considered every possible node. if path not found it does not exist
                    ...
                    for row in grid:
                        for spot in row:
                            spot.update_neighbors(
                                grid)  ##update neighbros if space is pressed and we've not started algorithm
